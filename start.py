...
db_path = os.path.join(os.sep, 'app', 'db', 'kms.db')   # 定义数据库文件的路径
...
def start_kms(logger):
    ...
    try:
        if want_webui:
            ...
            pykms_webui_process = subprocess.Popen(
                ['gunicorn', '--log-level', os.environ.get('LOGLEVEL'), 'pykms_WebUI:app'], env=pykms_webui_env)
